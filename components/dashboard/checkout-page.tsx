...
import { ArrowLeft, DollarSign, CheckCircle, AlertTriangle, Loader2 } from "lucide-react"
...
export function CheckoutPage({
  ...
  const [fundsToAdd, setFundsToAdd] = useState<number>(
    walletBalance < totalCost ? Math.ceil(totalCost - walletBalance) : 0,
  )
  ...
  const serviceFee = itemsTotal * 0.05 // 5% service fee
  const tax = itemsTotal * 0.08 // 8% tax
  const shipping = totalCost - itemsTotal - serviceFee - tax
          ...
          <div className="md:col-span-2 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review your shipping request details</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <Card className="max-w-md mx-auto">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="text-center text-sm text-muted-foreground">
              {processingProgress < 100 ? `Processing... ${processingProgress}%` : "Finalizing Order..."}
            </div>
          </CardContent>
        </Card>
      )}
